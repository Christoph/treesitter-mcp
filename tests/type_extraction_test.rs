...
fn extracts_go_structs_and_interfaces() {
    ...
    assert!(fields
        .iter()
        .any(|field| field.name == "X" && field.type_annotation == "int"));
    assert!(fields
        .iter()
        .any(|field| field.name == "Y" && field.type_annotation == "int"));

    let calculator = result
        .types
        .iter()
        .find(|t| t.name == "Calculator")
        .expect("Calculator interface should be extracted");
    assert_eq!(calculator.kind, TypeKind::Interface);

    let members = calculator
        .members
        .as_ref()
        .expect("interfaces should include members");
    assert!(members.iter().any(|member| member.name == "Add"));
    assert!(members.iter().any(|member| member.name == "Subtract"));
}
