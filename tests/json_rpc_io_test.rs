...
fn test_read_message_from_stdin() {
    ...
    let msg = treesitter_cli::mcp::io::read_message(&mut reader).unwrap();
...
fn test_write_message_to_stdout() {
    ...
    treesitter_cli::mcp::io::write_message(&mut output, &response).unwrap();
...
fn test_handle_invalid_json() {
    ...
    let mut reader = Cursor::new(&input[..]);
    let result = treesitter_cli::mcp::io::read_message(&mut reader);
...
fn test_read_multiple_messages() {
    ...
    let msg1 = treesitter_cli::mcp::io::read_message(&mut reader).unwrap();
    assert_eq!(msg1.method, "test1");

    let msg2 = treesitter_cli::mcp::io::read_message(&mut reader).unwrap();
